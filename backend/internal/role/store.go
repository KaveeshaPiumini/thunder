...
type roleStoreInterface interface {
	...
	IsRoleExist(ctx context.Context, id string) (int, error)
...
func (s *roleStore) IsRoleExist(ctx context.Context, roleID string) (int, error) {
	dbClient, err := s.getIdentityDBClient()
	if err != nil {
		return 0, err
	}
	...
	if len(result) > 1 {
		return 0, errors.New("multiple roles found")
	}
...
func (s *roleStore) DeleteRole(ctx context.Context, id int) error {
	...
	if rowsAffected == 0 {
		logger.Debug("Role not found with the given id.")
	}
